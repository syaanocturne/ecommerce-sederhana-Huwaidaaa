🍪 Mammalia — Home-cooked meals by Mama Lia
...
Live Demo: 
1. (https://syaanocturne.github.io/ecommerce-sederhana-Huwaidaaa/)
2. Admin (https://syaanocturne.github.io/ecommerce-sederhana-Huwaidaaa/ADMIN/)
3. Repository (https://syaanocturne.github.io/ecommerce-sederhana-Huwaidaaa/)
...
SegmenKarakteristikKebutuhanIndividu/Keluarga urbanUsia 20–45 tahun, tinggal di kota besar, aktif belanja onlineCamilan berkualitas untuk konsumsi harianPemberi hadiahIngin memberi hampers untuk Lebaran/Natal/ulang tahunKemasan menarik, opsi gift boxReseller/CorporateMembeli dalam jumlah besar untuk dijual kembali atau parsel kantorHarga grosir, konsistensi kualitasPecinta kuliner nicheMencari rasa unik seperti matcha atau red velvetVariasi rasa & rilis produk baru
...
Rentang HargaKategori ProdukRp 32.000 – Rp 45.000Cookies & Pastry satuanRp 120.000 – Rp 150.000Gift Box / Hampers
...
Katalog Produk → pelanggan menambahkan produk ke keranjang (Add to Cart), data
tersimpan di localStorage browser sehingga tetap ada meski halaman di-refresh.
Keranjang Belanja (cart.html) → pelanggan dapat mengubah kuantitas, menghapus
item, memasukkan kode promo, dan melihat total otomatis (subtotal + ongkir − diskon).
Checkout (checkout.html) → pelanggan mengisi form data pengiriman (nama, email,
no. HP, alamat, kota, kode pos) yang divalidasi di sisi client sebelum lanjut.
Pemilihan Metode Pembayaran → pelanggan memilih salah satu dari:
...
Simulasi Pembayaran → tombol "Bayar Sekarang" memicu proses simulasi (loading ±1.8
...
MetrikInsight yang DidapatKeputusan BisnisBounce rateHalaman mana yang cepat ditinggalkan pengunjungPerbaiki konten/hero di halaman dengan bounce rate tinggiConversion rate% pengunjung yang menyelesaikan checkoutEvaluasi hambatan di funnel checkoutAdd-to-cart rateRasio klik "Add to Cart" per kunjungan produkIdentifikasi produk yang menarik minat tapi kurang lakuCart abandonment rate% pelanggan menambah item tapi tidak checkoutKirim reminder / evaluasi ongkir & metode bayarAverage order value (AOV)Rata-rata nilai transaksiRancang bundling/upsell untuk menaikkan AOVProduk terlarisProduk dengan event add_to_cart/purchase terbanyakPrioritaskan stok & promosi produk populerTraffic sourceSumber kunjungan (organik, sosial media, direct)Alokasikan budget marketing ke kanal paling efektifSearch termsKata kunci yang sering dicari di search box internalKembangkan produk baru sesuai permintaan pasar
...
│   ├── kastangel.jpeg
│   └── *.svg / *.jpeg / *.jpg   # Foto produk lainnya (format bisa campur, lihat §15)
└── ADMIN/                # Panel admin "backend palsu" berbasis localStorage
    ├── login.html         # Login admin (dummy, lihat §14)
    ├── index.html         # Dashboard ringkasan
    ├── dashboard.html
    ├── products.html       # CRUD produk (tambah/edit/hapus, termasuk ganti foto)
    ├── product-form.html   # Form tambah/edit produk
    ├── orders.html         # Daftar & kelola status pesanan
    ├── tracking.html       # Info resi/pengiriman
    ├── css/admin.css
    └── js/
        ├── admin-auth.js     # Login/session (sessionStorage)
        ├── admin-data.js     # Load/save produk & pesanan (localStorage)
        ├── admin-layout.js   # Sidebar/layout bersama
        ├── admin-products.js
